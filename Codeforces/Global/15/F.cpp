...
void solve() {
    ...
    for (int i = 0; i < n; i++)
        cin >> x[i] >> y[i] >> s[i];
    pref[0] = 0;
    // dp[i] == time required to pass through portal i
    // all portals before i would be active at that time
    for (int i = 0; i < n; i++) {
        int pos = (int)(lower_bound(x.begin(), x.end(), y[i]) - x.begin());
        dp[i] = num(x[i]) - num(y[i]) + pref[i] - pref[pos];
        pref[i+1] = dp[i] + pref[i];
    }
    num res = x[n-1];
    for (int i = 0; i < n; i++) {
        if (s[i])
