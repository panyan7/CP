...
void solve() {
    ...
    cin >> s >> b >> k >> h;
    vector<int> cnt_s(n, 0), cnt_b(n, 0);
    vector<ll> x(s, 0), a(s, 0), f(s, 0);
    MaxFlow G(s+b+2, s+b, s+b+1);
    for (int i = 0; i < s; i++) {
        cin >> x[i] >> a[i] >> f[i];
        x[i]--;
        G.add_edge(s+b, i, 1);
    }
    vector<ll> y(b, 0), df(b, 0);
    for (int i = 0; i < b; i++) {
        cin >> y[i] >> df[i];
        y[i]--;
        G.add_edge(s+i, s+b+1, 1);
    }
    vector<vector<int>> cap(n, vector<int>(n, 0));
    for (int i = 0; i < s; i++) {
        for (int j = 0; j < b; j++) {
            if (a[i] >= df[j] && f[i] >= d[x[i]][y[j]]) {
                G.add_edge(i, s+j, 1);
            }
        }
    }
